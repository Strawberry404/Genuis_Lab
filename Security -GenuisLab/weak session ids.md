...
Modern frameworks implement safe, unguessable session IDs. If you are using a recent version of your web toolkit, check to see how the session IDs are generated. The code below demonstrate a n good ways to generate session IDs.

#### Tamper-Proof Your Cookies

Frameworks like [Rails](https://guides.rubyonrails.org/security.html#session-storage) and [Django](https://docs.djangoproject.com/en/3.1/topics/http/sessions/#using-cookie-based-sessions) allow you to sign your cookies. This means the server will be able to tell if the cookie has been manipulated since it was sent to the browser with the `Set-Cookie` header. Any indication of the data being tampered with will invalidate the session.
